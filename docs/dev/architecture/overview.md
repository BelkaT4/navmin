...
- `Config Manager` - Хранит все настройки в едином словаре

    Поддерживает загрузку/сохранение из файла (JSON). Оповещает подписанные модули об изменениях ***(через callback или очередь)***.

!!! warning "TODO"
    выбрать: callback или очередь
