...
В этом разделе описаны сквозные механизмы, которые обеспечивают работу всей системы: модель потоков и очередей, управление конфигурацией, обработка ошибок и режимы симуляции.

---

### Потоки и очереди сообщений

Система использует многопоточность для параллельной обработки данных и обеспечения отзывчивости интерфейса. Все потоки запускаются при старте приложения и завершаются корректно при выходе.

#### Список потоков

1. **Главный поток (UI)** – отвечает за отображение интерфейса, обработку событий пользователя и обновление виджетов. В нём работают все компоненты PyQt6.
2. **Поток захвата видео (Grabber)** – непрерывно получает кадры с трёх камер через `Camera Controller`-ы и помещает их в очередь `frame_queue`. Работает в цикле с небольшой задержкой.
3. **Поток обработки (Vision Pipeline)** – забирает кадры из `frame_queue`, последовательно запускает детектор и трекер, а затем публикует результат (`DetectedObject`) в очередь `object_queue`. Также обрабатывает стереозрение, если оно включено.
4. **Поток управления турелью (Turret Controller)** – слушает очередь `command_queue`, выполняет команды (наведение, калибровка, остановка), отправляет команды через `Turret HAL` и публикует состояние в `status_queue`.

Все потоки (кроме главного) являются **демонами** – они автоматически завершаются при закрытии приложения.

#### Очереди сообщений (сводная таблица)

| Очередь | Производитель | Потребитель | Тип данных | Назначение |
|---------|---------------|-------------|------------|------------|
| `frame_queue` | Поток захвата видео | Поток обработки | `numpy.ndarray` (или пара кадров для стерео) | Передача сырых кадров для детекции |
| `object_queue` | Поток обработки | Core (и через Core – Turret, UI) | `list[DetectedObject]` | Передача обнаруженных объектов |
| `command_queue` | Core (от UI или других модулей) | Поток управления турелью | `dict` (команда + параметры) | Команды управления турелью |
| `status_queue` | Поток управления турелью | Core (и через Core – UI) | `dict` (состояние турели) | Статус турели (положение, ошибки) |
| `ui_command_queue` | UI (главный поток) | Core | `dict` (команда + параметры) | Команды от пользователя (выбор цели, настройки) |
| `ui_status_queue` | Core | UI (главный поток) | `dict` (обновления состояния) | Обновления для отображения в UI |

Все очереди реализованы с помощью `queue.Queue` из стандартной библиотеки Python, что гарантирует потокобезопасность.

**Примечание:** Core не является отдельным потоком – он представляет собой набор функций и структур данных, которые используются потоками. При необходимости (например, для сложной маршрутизации) можно выделить отдельный поток-диспетчер, но на начальном этапе он не требуется.

---

### Управление конфигурацией (Config Manager)

`Config Manager` – это центральный компонент Core, отвечающий за хранение, загрузку и распространение настроек.

#### Структура хранения

- Все настройки хранятся в памяти в виде **единого словаря** (вложенная структура).
- При старте приложения конфигурация загружается из **JSON-файла** (путь указывается в аргументах командной строки или задаётся по умолчанию).
- При изменении настроек через UI файл автоматически обновляется (сохраняется).

#### Механизм оповещения об изменениях (паттерн «Наблюдатель»)

Представьте, что вы подписались на новости в мессенджере: как только выходит новая статья, вам приходит уведомление. В программе работает так же – модули могут **подписаться** на изменения в конфигурации, и когда какой-то параметр меняется, все подписанные модули получают уведомление и обновляют свои внутренние настройки.

- Подписка осуществляется через вызов метода `subscribe(section, callback)`.
- При изменении любого параметра в разделе `section` вызываются все зарегистрированные callback-функции с новым значением.
- Это позволяет модулям (Vision, Turret, UI) динамически применять новые настройки без перезапуска.
...
```json
{
  "vision": {
    ...
    "cameras": {
      "cam1": {"enabled": true, "detector": "ROIBasedDetector", "tracker": "KalmanTracker"},
      "cam2": {"enabled": true, "detector": "ROIBasedDetector", "tracker": "KalmanTracker"},
      "cam3": {"enabled": false, "detector": "SlidingWindowDetector", "tracker": "HungarianTracker"}
    }
  },
  "turret": {
    "port": "/dev/ttyUSB0",
...
Модули получают доступ к конфигурации через глобальный экземпляр `ConfigManager`, который инициализируется в Core. Каждый модуль при старте запрашивает нужные ему разделы и подписывается на обновления.

---

### Обработка ошибок и восстановление

Система должна быть устойчивой к сбоям отдельных компонентов. Для этого предусмотрены следующие уровни защиты.

#### 1. Переподключение при потере связи

- **Камеры:** `Camera Controller` автоматически пытается переподключиться к камере при обрыве соединения. Попытки повторяются бесконечно с экспоненциальной задержкой (1, 2, 4, 8 секунд и т.д.), пока соединение не восстановится. При восстановлении поток кадров возобновляется.
- **STM32:** `Turret HAL` при таймауте повторяет команду до `retry_count` раз. Если все попытки неудачны, в `status_queue` отправляется ошибка, и турель переходит в безопасное состояние (моторы останавливаются). Пользователь может инициировать повторную калибровку или перезапуск модуля через UI.

#### 2. Мониторинг потоков (Супервизор)

Core может отслеживать состояние потоков через **heartbeat** (периодические сообщения от потоков в специальную очередь или через таймауты). Если поток не отвечает в течение заданного времени (например, 5 секунд), Core:
- Логирует ошибку.
- Отображает предупреждение в UI.
- По желанию (настраивается) может автоматически перезапустить проблемный поток.

На начальном этапе супервизор реализуется в упрощённом виде – только логирование и уведомление пользователя. Автоматический перезапуск будет добавлен позже.

#### 3. Логирование

Все ошибки, предупреждения и информационные сообщения записываются в **лог-файл**. При каждом запуске приложения создаётся новый файл с именем, содержащим временную метку (например, `app_2026-09-02_15-30-45.log`). Это упрощает анализ проблем, так как логи разных сеансов работы не смешиваются.

- В режиме разработки логи выводятся также в консоль.
- В продакшене – только в файл.
- UI отображает последние сообщения об ошибках (кратко) в виде уведомлений.

#### 4. Безопасное завершение

При закрытии приложения все потоки корректно останавливаются:
- Потоку захвата видео отправляется сигнал остановки.
- Турель останавливается (команда `STOP`).
- Все очереди опустошаются.
...
Для отладки и тестирования без реального оборудования предусмотрены два режима симуляции.

#### 1. Симуляция видеопотока (без камер)

Вместо подключения к реальным Raspberry Pi камерам `Camera Controller` может читать кадры из:
- **Видеофайла** (например, `.mp4` или `.avi`) – циклическое воспроизведение.
- **Набора изображений** в папке – последовательный показ.
- **Синтетического генератора** – программная генерация простых сцен (например, движущийся объект на однородном фоне).
- **Виртуальной OBS-камеры** – если на компьютере установлен OBS Studio с включённым виртуальным выходом, то видеопоток можно получать как от обычной камеры через интерфейс DirectShow или V4L2 (в зависимости от ОС). Это удобно для демонстрации и тестирования с живым видео, генерируемым OBS (например, наложение графики, смена сцен).

Режим симуляции включается параметром в конфигурации (`simulation_mode: true`) или через аргумент командной строки. Это позволяет разрабатывать и тестировать детектор и трекер без физических камер.

#### 2. Симуляция STM32 (эмулятор турели)

Вместо реального последовательного порта `Turret HAL` может работать с **эмулятором**, который:
- Принимает команды (как STM32) и возвращает фиктивные ответы.
- Имитирует движение моторов с заданной задержкой и шумом.
- Позволяет проверить логику `Turret Controller` (PI-регулятор, компенсацию люфта) без подключения к реальному железу.

Эмулятор также включается параметром конфигурации (`emulate_stm32: true`). Он реализован как отдельный класс, который подменяется вместо `Turret HAL` при инициализации.

#### Использование симуляции

Оба режима могут использоваться независимо или вместе. Это даёт возможность:
- Тестировать всю систему в среде, максимально приближенной к реальной, но без оборудования.
- Быстро проверять изменения в алгоритмах.
- Обучать оператора на симуляторе.

Симуляция не требует изменения кода модулей – только изменение конфигурации, что делает её удобной для CI/CD и автоматического тестирования.
